...
logging.basicConfig(filename='wd\study_plans\code_for_others.log', format='%(asctime)s - %(levelname)s - %(message)s')
...
def extract_coords_from_pdf(pdf_path, search_string):
    ...
    try:
        ...
        logging.error("Error while processing file {%s} Exception occurred", pdf_path[pdf_path.rindex(rf"\")+1:] exc_info=True)
