...
logging.basicConfig(filename=log_file_name, format='%(asctime)s - %(levelname)s - %(message)s', level=logging.de)
                    ...
                    for element in layout:
                        if isinstance(element, LTTextBox):
                            ...
                            if element.get_text().strip() == search_string:
                                ...
                                logging.info("Year 1 found with coords {%s} at page {%s}", data[0:-2], data[-1])
                                logging.info("ENXT LOGGING\n")
                                    ...
                                    for obj in layout_1._objs:
                                        if isinstance(obj, LTTextBox) or isinstance(obj, LTTextLine):
                                            ...
                                            if xmin_condition and ymin_condition and xmax_condition and ymax_condition:
                                                ...
                                                logging.info("Renamed file {%s}, to {%s} in directory {%s}", old_name, new_filename, dir_name)
                                                logging.info("NEXT LOGGING \n")
